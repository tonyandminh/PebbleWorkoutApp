...
void menu_draw_header_callback(GContext* ctx, const Layer *cell_layer, uint16_t section_index, void *data) {
    menu_cell_basic_header_draw(ctx, cell_layer, "Workouts");
}
    ...
    switch(cell_index -> section){
        case 0: 
            switch(cell_index -> row){
                case 0:
                    menu_cell_basic_draw(ctx, cell_layer, "Atlanta", "79 F", NULL);
                    break;
            }
            break;
        case 1:
...
void main_window_destory(){
    window_destroy(mainWindow);
}

void *main_window_get_window(){
    return mainWindow;
}